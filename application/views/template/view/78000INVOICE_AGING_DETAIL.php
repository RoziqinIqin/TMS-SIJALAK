...
<form>
<table cellpadding="1" cellspacing="1" width="1500">
  <tr bgcolor="#CDCBBF" align="center">
    ...
    <td colspan=2 >Last Follow Up Status</td>
...
<?
  ...
  foreach ($viewrs as $rviewrs) {
    ...
    if ($tot_balance_idr > 0 || $tot_balance_usd > 0) {
      $customer_group[] = $rviewrs->Customer;
    }
  } 
  $customer = array_unique($customer_group);
  foreach ($customer as $rcustomer) {
    ${"count" . $rcustomer} = 0;
    echo "<tr bgcolor='#CDCBBF'>";
    echo "<td colspan=16><b>$rcustomer</b></td>";
    echo "</tr>";
 
    foreach ($viewrs as $rviewrs) {
      if ($rviewrs->Customer == $rcustomer) {
        $tot_payment_idr = 0;
        $tot_dp_idr = 0;
        $tot_payment_usd = 0;
        $tot_dp_usd = 0;
        ...
        if ($tot_balance_idr > 0 || $tot_balance_usd > 0) {
          ...
          ${$rcustomer . "invoiceIDR"} = ${$rcustomer . "invoiceIDR"} + $rviewrs->IDR;
          ${$rcustomer . "invoiceUSD"} = ${$rcustomer . "invoiceUSD"} + $rviewrs->USD;
          ${$rcustomer . "DPIDR"} = ${$rcustomer . "DPIDR"} + $tot_dp_idr;
          ${$rcustomer . "DPUSD"} = ${$rcustomer . "DPUSD"} + $tot_dp_usd;
          ${$rcustomer . "paymentIDR"} = ${$rcustomer . "paymentIDR"} + $tot_payment_idr;
          ${$rcustomer . "paymentUSD"} = ${$rcustomer . "paymentUSD"} + $tot_payment_usd;
          ${$rcustomer . "balanceIDR"} = ${$rcustomer . "balanceIDR"} + $tot_balance_idr;
          ${$rcustomer . "balanceUSD"} = ${$rcustomer . "balanceUSD"} + $tot_balance_usd;
	  ### Follow Up Activity 
          $follow_up = $this->db->query("select t0.fld_btdesc,t0.fld_btdt from tbl_bth_arfup t0 
					 where t0.fld_btreffid=$rviewrs->fld_btid order by t0.fld_btid DESC limit 1");
          $follow_up = $follow_up->row();
            $fupdt = "<a href='" . base_url() . "index.php/page/view/78000ARFUP?btreffid=" . $rviewrs->fld_btid . "'>$follow_up->fld_btdt</a>";
          ...
          echo "<td><a href='" .  base_url() . "index.php/page/form/78000ARFUP/add/?fld_btreffid=" . $rviewrs->fld_btid  . "'>" . $rviewrs->fld_btno . "</a></td>";
          ...
          echo "<td>" . $fupdt . "</td>";
          echo "<td width='250'>" . $follow_up->fld_btdesc . "</td>";
          echo "</tr>";
        }
      }
    }
    ...
    echo "<td align='right'>" . number_format(${$rcustomer . "invoiceIDR"},2,',','.') . "</td>";
    echo "<td align='right'>" . number_format(${$rcustomer . "invoiceUSD"},2,',','.') . "</td>";
    echo "<td align='right'>" . number_format(${$rcustomer . "DPIDR"},2,',','.') . "</td>";
    echo "<td align='right'>" . number_format(${$rcustomer . "DPUSD"},2,',','.') . "</td>";
    echo "<td align='right'>" . number_format(${$rcustomer . "paymentIDR"},2,',','.') . "</td>";
    echo "<td align='right'>" . number_format(${$rcustomer . "paymentUSD"},2,',','.') . "</td>";
    echo "<td align='right'>" . number_format(${$rcustomer . "balanceIDR"},2,',','.') . "</td>";
    echo "<td align='right'>" . number_format(${$rcustomer . "balanceUSD"},2,',','.') . "</td>";
