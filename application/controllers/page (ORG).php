...
class Page extends CI_Controller {
  ...
  function form_process() {
    ...
    if ($mode == 'add' || $mode == 'edit'){
       ...
       #  if($post_data['fld_btp02'] == 1 && ($post_data['fld_btp04'] == "" || $post_data['fld_btp05'] || $post_data['fld_btp06'])) {
       #    $this->dnxapps->message("Data Armada Subcon yang anda masukkan belum lengkap ...");
       #  } else if($post_data['fld_btp02'] != 1 &&($post_data['fld_btp12'] == "" || $post_data['fld_btp11'] == "")) {
       #    $this->dnxapps->message("Data Armada Milik yang anda masukkan belum lengkap ...");
       #  }
    ...
    if ($tynextform == '78000DELIVERY_ORDER_BOX') {
      $this->dnxapps->setCashAdvance($fup_lid,$this->input->post(fld_baidc),$this->input->post(fld_btp09),$this->input->post(fld_btflag),$this->input->post(fld_baidv),$this->input->post(fld_btloc),$this->input->post(fld_btflag),$this->input->post(fld_btp18));
    }
    
    if ($tynextform == '78000RETURN_DO') {
        $this->db->query("update tbl_bth t0 set t0.fld_btp24=(select fld_trfamt from tbl_trf where fld_btiid=t0.fld_btp09 and fld_trfp01=t0.fld_btp26 and fld_beid=t0.fld_baidc)
...
//	}
  ...
  function printDOTruck() {
    $fld_btid =  $this->uri->segment(3);
    $printDOTruck = $this->dnxapps->printDOTruck($fld_btid);
  }
  ...
  function cancelOrder() {
    $node =  $this->uri->segment(3);
    $fld_baidp  =$this->session->userdata('userid');
    $this->db->query("update tbl_bth set fld_btstat = 5 where fld_btid=$node and fld_baidp = $fld_baidp limit 1");
    echo '<script>history.go(-1)</script>';
  }
  ...
  function deleteTransfer() {
    ...
    $this->db->query("update tbl_bth set fld_btstat = 1 where fld_btid=$node3");
  ...
  function truckingSetBilling(){
    ...
    for ($x=1; $x<=$count; $x++){
     ...
     $this->db->query("update tbl_bth set 
                       fld_btamt=$a,
                       fld_btuamt=$c,
                       fld_bttax = if(fld_btp01 = 1 , fld_btamt * 0.1,0),
  ...
  function CloseTRF($btid)
  {
	  ...
	  {
		 $this->db->query("update tbl_bth set  fld_btstat = 4 where fld_btid in (select fld_btreffid from tbl_trk_transfer where fld_btidp='$btid')"); 
		 $this->db->query("update tbl_bth set  fld_btstat = 3 where fld_btid='$btid'"); 
