...
### Change by Cenot 22 Okt 2015    
    ...
    $this->cezpdf->addText(30,530,10,"Dengan ini kami sampaikan tagihan biaya angkutan sebesar, Rp. " .$data->amount);

	### $this->cezpdf->addText(30,515,10,"Rp. " . $data->amount);
    $this->load->helper("terbilang");
    //$terbilang = ucwords(number_to_words($data->fld_btbalance));
    $terbilang = str_replace("  "," ",ucwords(number_to_words($data->fld_btbalance)));
    $terbilang3 = "";
	$max_kar = 85;
...
### Change by Cenot 22 Okt 2015    
    ...
    $this->cezpdf->addTextWrap(400,330,200,10,$data->ttd,'center');
    $this->cezpdf->addTextWrap(400,315,200,10,"( " . $data->jabatan . " )",'center' );
     
    header("Content-type: application/pdf");
    header("Content-Disposition: attachment; filename=SuratPengantar-" . date('Ymd') .  ".pdf");
  ...
  function printINV2($fld_btid) {
   ...
   'amount'=>array('width'=>80,'justification'=>'right'),
    ...
    header("Content-type: application/pdf");
    header("Content-Disposition: attachment; filename=RekapAngkutan-" . date('Ymd') .  ".pdf");
    header("Pragma: no-cache");
    header("Expires: 0");
...
###1. Change by Cenot 22 Okt 2015    
    ...
    if ($jml_terbilang < $max_kar) 
		$this->cezpdf->addText(150,720,10,"# ".$cetak."Rupiah #");
    else {
...
###1. Change by Cenot 22 Okt 2015    
    ...
    $this->cezpdf->addText(480,670,10,"Rp. " . $data->amount );
...
###2. Change by Cenot 22 Okt 2015
    ...
    $this->cezpdf->addText(480,405,10,"Rp. " . $data->amount );
...
###3. Change by Cenot 22 Okt 2015
    ...
    $this->cezpdf->addText(480,140,10,"Rp. " . $data->amount );
    ...
    header("Content-type: application/pdf");
    header("Content-Disposition: attachment; filename=Kwitansi-" . date('Ymd') .  ".pdf");
    header("Pragma: no-cache");
    header("Expires: 0");
  ...
  function printINV4($fld_btid) {
    ...
    */
    ...
    header("Content-type: application/pdf");
    header("Content-Disposition: attachment; filename=FakturPajak-" . date('Ymd') .  ".pdf");
    header("Pragma: no-cache");
    header("Expires: 0");
  ...
  function printINV5($fld_btid) {
   ...
   'amount'=>array('width'=>50,'justification'=>'right'),
    ...
    header("Content-type: application/pdf");
    header("Content-Disposition: attachment; filename=ProformaInvoice-" . date('Ymd') .  ".pdf");
    header("Pragma: no-cache");
    header("Expires: 0");
...
#main right
  ...
  */
  ...
  function exportTransfer($fld_btid) {
   ...
   echo "Cabang;Pembuat SO;Armada;Nomor Kendaraan;Customer;Nomor SO;Tanggal;Jenis SO;Rute;Jenis Biaya;Supir;No. HP;Nama Bank;No. Rekening;Jumlah \n";
   $cash = 0;
   $save = 0;
   foreach($data->result() as $rdata) {
     echo "$rdata->location;$rdata->fld_username;$rdata->FleetType;$rdata->VehicleNumber;$rdata->Customer;$rdata->DONumber;$rdata->SODate;$rdata->jenis;$rdata->fld_btp04;$rdata->fld_trk_settlementtype ;$rdata->Driver;$rdata->fld_btp01;$rdata->fld_btp02;$rdata->fld_btp03;" . number_format($rdata->Cash,2,',','.') . ";" . "\n";
     $cash = $cash + $rdata->Cash;
   }
